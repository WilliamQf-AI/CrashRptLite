...
struct ERIFileItem {
  ...
  // Destination file name as it appears in ZIP archive (not including directory name).
  CString m_sDestFile;
...
struct ERIRegKey {
  ERIRegKey() { m_bAllowDelete = false; }
...
class CErrorReportInfo {
 ...
 private:
  ...
  std::map<CString, ERIFileItem>
      m_FileItems;  // The list of files that are included into this error report.
  std::map<CString, ERIRegKey>
      m_RegKeys;  // The list of registry keys included into this error report.
  std::map<CString, CString>
      m_Props;  // The list of custom properties included into this error report.
};
...
class CCrashInfoReader {
 public:
  ...
  BOOL
      m_bClientAppCrashed;  // If TRUE, the client app has crashed; otherwise the client app exited successfully.
