...
int Utility::GetGeoLocation(CString& sGeoLocation) {
  ...
  HMODULE hKernel32 = LoadLibrary(_T("kernel32.dll"));
  if (hKernel32 != NULL) {
    PFNGETUSERGEOID pfnGetUserGeoID = (PFNGETUSERGEOID)GetProcAddress(hKernel32, "GetUserGeoID");
    PFNGETGEOINFOW pfnGetGeoInfoW = (PFNGETGEOINFOW)GetProcAddress(hKernel32, "GetGeoInfoW");
...
CString Utility::GetProductVersion(CString sModuleName) {
  ...
  if (NULL != pBuff && 0 != GetFileVersionInfo(sModuleName, 0, dwBuffSize, pBuff)) {
    ...
    sProductVer.Format(_T("%u.%u.%u.%u"), dwVerMajor, dwVerMinor, dwPatchLevel, dwVerBuild);
  }
